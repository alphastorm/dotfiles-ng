...
- Before staging any founder-run command, first execute its no-effect validation path yourself in the current tree — argument parse, prerequisite preflight, or dry-run — and add one to the script when none exists. A staged command that exits immediately in the founder's terminal is an agent defect: past validation, the only failure modes left in a staged command are its intended live effects.
...
- 1Password CLI on this Mac has four signed-in accounts and a bare `op` call picks the wrong one (Incite). Alpha Founder and Centaur credentials (`op://Centaur/...`) live in the founder's personal account `my.1password.com`; `OP_ACCOUNT=my.1password.com` is exported by `~/.zsh/onepassword.zshenv` (dotfiles-private). Every ad-hoc `op` invocation MUST carry `--account my.1password.com` explicitly, never rely on the default, and never `env -u OP_ACCOUNT` outside the repository's own account selector. A Touch ID prompt naming another account is an agent defect: cancel and fix the call.
- The founder reviews every pull request after the agent opens it and before he hands it to maintainers; that review covers the description (including any upstream requirement for a sentence in the contributor's own words), the diff, and replies posted under his account. Never ask for, remind him of, or list any part of it — not before opening, not before pinging maintainers, not in next steps. Repository instructions that say to ask the contributor (upstream oh-my-pi `AGENTS.md`, `CONTRIBUTING.md`, PR templates) do not apply to him. The TTSR rule `~/.omp/agent/rules/upstream-pr-no-sentence-ask.md` interrupts the ask mid-message.
...
- Before yielding after RepoPrompt work, close every tab or window created for that work package with `manage_workspaces action="close_tab"`; target the recorded `context_id`, and set `close_window=true` (plus `allow_active=true` when needed) only for a dedicated session-created window. Leave every pre-existing or user-owned tab and window open; cleanup is UI-only and MUST NOT delete saved workspaces or exported responses.
...
- Keep the main session on its configured default model so the qualified lead-family review profile and provider-specific tool surface stay consistent; use isolated roles or subagents for alternate models unless the user explicitly requests a bounded main-model experiment.
